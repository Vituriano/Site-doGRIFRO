...
<body style="background:#eaeaea91;">
	...
	<div class="container">
		...
		while ($enquete = mysqli_fetch_array($query)) { 
			...
			<form id="form1" method="post" action="">
				<div class="list-group">
					...
					<div class="d-flex w-100 justify-content-between">
						<h5 class="mb-2"><?php echo $enquete["titulo"]; ?></h5>
					</div>	
					<p class="mb-1 text-muted"><?php echo $enquete["texto"]; ?></p>
				</div>
				<INPUT TYPE="hidden" NAME="idEnquete" VALUE="<?php echo $enquete['idEnquete']; ?>" />
				<button name="1" class='btn btn-danger' id="myBtn<?php echo $enquete['idEnquete'];?>" style='color: #FFF;'>Excluir</button>
			</a>
			<hr>
		</form>	
	<?php } 	
	if (isset($_POST["idEnquete"])) {
		$id = $_POST['idEnquete'];
		?>
		<form method="post" action="deleteenquete.php">
					...
					<center>
						<div style="width: 50%;">
							<p class='text-muted'>Tem certeza que deseja Excluir?<br><br>
								<span>
									<INPUT TYPE="hidden" NAME="idEnquete" VALUE="<?php echo $id; ?>" />
