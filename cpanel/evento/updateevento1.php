...
<body style="background:#eaeaea91;">
	...
	<div style="background-color: white;">
		...
		<ul class="container nav nav-tabs">
			...
			<li class="nav-item dropdown">
				<a class="nav-link dropdown-toggle active" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">Eventos</a>
				<div class="dropdown-menu">
					<a class="dropdown-item" href="inserirevento.php">Inserir Evento</a>
					<a class="dropdown-item" href="editarevento.php">Editar Evento</a>
					<a class="dropdown-item" href="excluirevento.php">Remover Evento</a>
				</div>
			</li>
			<li class="nav-item dropdown">
	...
	<div class="container">
		...
		$data_A = substr($data, 0, 4);
		$data = $data_D."/".$data_M."/".$data_A;


		$texto = $_POST["texto"];
		$titulo1 = "";
		$search = "SELECT idEventos FROM eventos WHERE titulo = '$titulo'";
		...
		if(!$validations or $a == $b){
			$sql_unlike = "SELECT * FROM eventos WHERE idEventos = '$id'";
			...
			$query = "UPDATE eventos SET titulo = '$titulo', data = '$data', imagem = '$novo_nome', texto = '$texto', atualizacao = '$atualizacao' WHERE idEventos = '$id'";

			$texto = nl2br($texto);

			if (mysqli_query($con, $query)) {

				while ($evento = mysqli_fetch_array($query_unlike)) {
					$tituloo = $evento['titulo'];
					$imagem = $evento['imagem'];
					unlink("paginas/".$tituloo.".php");
					unlink("../uploadsImages/".$imagem);
				}
				$sql = "SELECT * from eventos where idEventos = (select max(idEventos) from eventos)";
				...
				while (".$noticia." = mysqli_fetch_array(".$query.")) {
					".$id1." = ".$noticia."['idEventos'];
					".$titulo1." = ".$noticia."['titulo'];
					".$subtitulo1." = ".$noticia."['data'];
				...
				<p class='subtitulo'><b>O evento ocorrerá em <?php echo $subtitulo1;?>, podendo haver alterações</b></p>
				...
				printf(" <center>
					...
					<a  style='float:right;' href='editarevento.php'>
					</center>
					<button type='button' class='btn btn-success'>Continuar</button>
					</a>
					<br><br>", $titulo, $data, $novo_nome, $texto);
			...
			echo '<script type="text/javascript">alert("Não foi possivel fazer as mudanças, existe um evento com mesmo título já cadastrado");</script>';
			echo'<META HTTP-EQUIV=Refresh CONTENT="0; URL=editarevento.php">';
		}
		?>
